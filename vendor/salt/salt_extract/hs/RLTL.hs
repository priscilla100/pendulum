...
module RLTL (Expr(..), getSI, getLeftSI)where

-- This module defines the data structure for the syntax tree
-- in RLTL, i.e. LTL with accepton, rejecton and stopon operators.
...
data Expr 	= Ident SI String 
			...
			-- accept/reject/stop operators
			| Reject SI Expr Expr 
			| Accept SI Expr Expr 
			| StopExclCheck FP SI Expr Expr
			| StopExcl FP SI Expr Expr
			| StopIncl FP SI Expr Expr
...
instance Show Expr where	
  ...
  show (StopExclCheck fp _ e1 e2) = "(" ++ (show e1) ++ " stopon" ++ (show fp) ++ "_excl " ++ (show e2) ++ ")"
  show (StopExcl fp _ e1 e2) = "(" ++ (show e1) ++ " stopon" ++ (show fp) ++ "_excl " ++ (show e2) ++ ")"
  show (StopIncl fp _ e1 e2) = "(" ++ (show e1) ++ " stopon" ++ (show fp) ++ "_incl " ++ (show e2) ++ ")"
...
getSI (StopExcl _ si _ _) = si
getSI (StopExclCheck _ si _ _) = si
getSI (StopIncl _ si _ _) = si
getSI (Ident si _) = si
getSI (TT si) = si
getSI (FF si) = si
getSI (Or si _ _) = si
...
-- Returns the source code position info contained in an expression
getLeftSI :: Expr -> SI
getLeftSI (Reject si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (Accept si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (StopExcl _ si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (StopExclCheck _ si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (StopIncl _ si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (Ident si _) = si
getLeftSI (TT si) = si
getLeftSI (FF si) = si
getLeftSI (Or si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (And si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (Not si e1) = minSI2 si (getSI e1)
getLeftSI (Impl si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (Equ si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (Until _ si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (WeakUntil _ si e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (Next _ si e1) = minSI2 si (getSI e1)
getLeftSI (Always _ si e1) = minSI2 si (getSI e1)
getLeftSI (Eventually _ si e1) = minSI2 si (getSI e1)
getLeftSI (TPredict _ si r e1) = minSI2 si (getSI e1)
getLeftSI (TUntil _ si r e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (TWeakUntil _ si r e1 e2) = minSI3 si (getSI e1) (getSI e2)
getLeftSI (TAlways _ si r e1) = minSI2 si (getSI e1)
getLeftSI (TEventually _ si r e1) = minSI2 si (getSI e1)
getLeftSI (Error si s) = si

minSI2 (Position l1 c1) (Position l2 c2) =
  if (l1 < l2) then (Position l1 c1)
  else if (l2 < l1) then (Position l2 c2)
  else if (c1 < c2) then (Position l1 c1)
  else (Position l2 c2)
minSI3 si1 si2 si3 = minSI2 (minSI2 si1 si2) si3
